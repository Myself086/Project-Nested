...
Interpret_LoadIndirect:
	...
	sta	$_Memory_NesBank
	tax
	jmp	($_Interpret_LoadIndirect_Switch,x)

Interpret_LoadIndirect_Switch:
	.fill16	0x10, _Interpret_LoadIndirect_Switch_00
	.fill16	0x10, _Interpret_LoadIndirect_Switch_20
	.fill16	0x10, _Interpret_LoadIndirect_Switch_40
	.fill16	0x10, _Interpret_LoadIndirect_Switch_60
	.fill16	0x10, _Interpret_LoadIndirect_Switch_80
	.fill16	0x10, _Interpret_LoadIndirect_Switch_a0
	.fill16	0x10, _Interpret_LoadIndirect_Switch_c0
	.fill16	0x10, _Interpret_LoadIndirect_Switch_e0

Interpret_LoadIndirect_Switch_00:
	plx
	rtl
Interpret_LoadIndirect_Switch_20:
Interpret_LoadIndirect_Switch_40:
	ora	#0x1f
	trap
	Exception	"Indirect IO Access{}{}{}An indirect load was requested to page range 0x{x:X}-0x{a:X}."
Interpret_LoadIndirect_Switch_60:
	lda	#0xb0
	pha
	plb
	plx
	rtl
Interpret_LoadIndirect_Switch_80:
	lda	$_Program_Bank_0+2
	pha
	plb
	plx
	rtl
Interpret_LoadIndirect_Switch_a0:
	lda	$_Program_Bank_1+2
	pha
	plb
	plx
	rtl
Interpret_LoadIndirect_Switch_c0:
	lda	$_Program_Bank_2+2
	pha
	plb
	plx
	rtl
Interpret_LoadIndirect_Switch_e0:
...
Interpret_StoreIndirect:
	...
	sta	$_Memory_NesBank
	tax
	jmp	($_Interpret_StoreIndirect_Switch,x)

Interpret_StoreIndirect_Switch:
	.fill16	0x10, _Interpret_StoreIndirect_Switch_00
	.fill16	0x10, _Interpret_StoreIndirect_Switch_20
	.fill16	0x10, _Interpret_StoreIndirect_Switch_40
	.fill16	0x10, _Interpret_StoreIndirect_Switch_60
	.fill16	0x10, _Interpret_StoreIndirect_Switch_80
	.fill16	0x10, _Interpret_StoreIndirect_Switch_a0
	.fill16	0x10, _Interpret_StoreIndirect_Switch_c0
	.fill16	0x10, _Interpret_StoreIndirect_Switch_e0

Interpret_StoreIndirect_Switch_00:
	plx
	rtl
Interpret_StoreIndirect_Switch_60:
	lda	#0xb0
	pha
	plb
	plx
	rtl
Interpret_StoreIndirect_Switch_80:
Interpret_StoreIndirect_Switch_a0:
Interpret_StoreIndirect_Switch_c0:
Interpret_StoreIndirect_Switch_e0:
Interpret_StoreIndirect_Switch_20:
Interpret_StoreIndirect_Switch_40:
	ora	#0x1f
	trap
...
Interpret__RtsI:
	...
	lda	$6,s
	inc	a
	bra	$+Interpret__RtsI_in

	// Timing:
	//  9.66 from precall if in ROM (11.00 in RAM)
	//  68.66 between start and loop (not counting last TAY)
	//  11.66 from successful attempt in the loop
	//  19.33 per failed attempt in the loop
	//  47.66 from returning
	// Fastest possible time: 137.66
	// Average time from RAM (not exact formula): 139.00 + ((number_of_known_destinations - 1) / 26.487) cycles
	.mx	0x30
	Interpret_Misalign
Interpret__JmpI:
	.vstack		_VSTACK_START
	//        1, 2, 3  4  5  6-7
	// Stack: y, x, a, p, k, indirectJumpPointer
	...
	// Zero Y for after changing mode
	ldy	#0x00
...
Interpret__RtsI_in:
	...
	tay
	lda	$_IO_Temp16+1
	ora	$_Program_BankNum-1,y
	tay

Interpret__JmpI_NodeLoop:
		// Node loop macro
		.macro	Interpret__JmpI
			// Is this the node that we're looking for?
			{0}tya
			eor	$=JMPi_Bank,x
			beq	$+Interpret__JmpI_Found

			// Load address for next node
			lda	$=JMPi_Bank+5,x
			//beq	$+Interpret__JmpI_NotFound
			tax
		.endm
		...
		// Loop back if we're still searching
		tya
		cpx	#_JMPi_EmptyPointer+2
		bne	$-Interpret__JmpI_NodeLoop
		beq	$+Interpret__JmpI_NotFound

Interpret__JmpI_Found:
	lda	$=JMPi_Bank+2,x
	sta	$5,s
	lda	$=JMPi_Bank+4,x
	sep	#0x31
...
Interpret__JmpI_OLD:
	.vstack		_VSTACK_START
	//        1  2  3  4-5
	// Stack: a, p, k, indirectJumpPointer
	...
	lda	($4,s),y
	Recompiler__CallFunctionJMPi_mac	"sta $3,s", "sta $4,s"
